        ...
        {analysis ? (
          <>
            ...
            <p className="text-xs text-slate-400 mt-3">
              Actions decided by {s.provider === "mock" ? "deterministic heuristics (demo mode)" : (s.provider ?? "unknown")}.{" "}
              Autopsy narrative generated by{" "}
              {analysis.provider === "mock" ? "deterministic heuristics (demo mode)" : analysis.provider}
              {!!analysis.fallback && (
                <strong className="text-amber-700">
                  {" "}— Gemini autopsy generation failed and fell back to the deterministic mock
                  narrative{analysis.fallback_reason ? ` (${analysis.fallback_reason})` : ""}. This
                  text was NOT generated by Gemini.
                </strong>
              )}
